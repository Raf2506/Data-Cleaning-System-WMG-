...
This commerce system is built on a single, almost violently simple idea: photography speaks, the chrome doesn't. Every page reads as an athletic editorial — towering uppercase display lockups (`{typography.display-campaign}`) set into full-bleed campaign imagery, with everything else (nav, filters, buttons, cards, footer) reduced to neutral typography and pill geometry on `{colors.canvas}` and `{colors.soft-cloud}`. There is no decorative gradient, no soft shadow nostalgia, no accent color used for "tone" — the system saves all chromatic energy for product photography and the small handful of moments that actually need to signal (sale price `{colors.sale}`, success `{colors.success}`, swatch dots).
...
Across the system's core templates — a gendered landing page, a trail-running listing, a footwear product page, a membership page, and a performance sub-line landing page — the same chrome appears in identical proportions — only the photography and copy change. That is the system's signature: maximum editorial expression in the imagery, maximum mechanical restraint everywhere else.

**Key Characteristics:**
- Editorial campaign hero with `{typography.display-campaign}` (96px, line-height 0.9, uppercase) set directly into full-bleed photography
...
> **Coverage:** The chrome palette is identical across the system's core templates — only photography varies.

### Brand & Accent
- **Primary Black** (`{colors.ink}` — `#111111`): The brand's only "color." It is the primary CTA, the swatch dot, the active filter chip, the campaign overlay, the headline color, and the body text. When the brand wants to assert anything, it goes black.
...
- **Accent Pink** (`{colors.accent-pink}` — `#ed1aa0`): Seasonal collaboration / women's collection moments.
...
- **Accent Teal** (`{colors.accent-teal}` — `#0a7281`): Trail / outdoor performance-line editorial accent in lockups.
- **Accent Pink Deep** (`{colors.accent-pink-deep}` — `#4c012d`): Deepest editorial overlay tint, used as wash on heritage / signature-line tiles.

## Typography

### Font Family
- **Anton** (display campaign only) — free geometric condensed sans for the towering uppercase headlines set into campaign hero photography. Falls back to Inter Display Bold → Arial Black → Arial.
- **Inter Display** (headings 16–32px) — display-tuned cut carrying every section title, PDP product name, and dialog headline.
- **Inter, weight 500** (UI 12–16px) — buttons, captions, swatch labels, badge text. The system's UI workhorse.
- **Inter, weight 400** (body and links) — long-form body and underlined inline links.
- **Noto Sans Arabic** — RTL pairing for Arabic locales at `{typography.heading-lg}` and caption sizes.
- **Inter 9px** — legal-fine-print utility row only (`{typography.utility-xs}`).

### Hierarchy

| Token | Size | Weight | Line Height | Letter Spacing | Use |
|---|---|---|---|---|---|
| `{typography.display-campaign}` | 96px | 500 | 0.9 | 0 | Editorial campaign headline set into hero photography (uppercase) |
...
### Principles
The system runs on extreme typographic contrast: a single 96px uppercase display tier reserved for editorial campaign moments, and a quiet 12–16px UI-text tier carrying everything else. There is almost no middle ground — the jump from `{typography.heading-xl}` (32px) directly to `{typography.body-strong}` (16px) is intentional and creates the "billboard above, catalog below" effect across every page. Letter-spacing is left at 0 (both the display and UI faces are cut for tight optical fit at scale).
...
- **Column patterns:** PLP listing uses 3-up at desktop, collapsing to 2-up at 1023px and 1-up at 599px. The men's home mixes a 2-up campaign hero row, a 3- or 4-up "Trending Now" row, a horizontal-scroll "Shop by Sport" rail, and a 4-up "Latest in Clothing" thumbnail grid.
...
### Decorative Depth
Depth in this system comes entirely from photography, not from CSS effects:
...
- **Editorial campaign hero:** ~16:9 or wider cinematic crop, full-bleed across the content max-width, with the display headline set into the lower-left or upper-left third.
...
- Used on every primary action in the system: "Sign Up", "Notify Me", "Buy", geo-confirm, "Shop" overlay on sport tiles, "Continue".
- Pressed state lives in `button-primary-active` — the bg stays `{colors.ink}` while the surface shrinks to `scale(0.5)` with `opacity: 0.5` (the system's signature "tap collapse" feedback that recurs across every template).
...
- Used as the lower-emphasis alternate when a primary CTA already exists, e.g., a geo-decline option next to a black geo-confirm; "Cancel" or "Discover More" on light cards.
...
**`campaign-tile`** — the system's signature editorial unit
- Full-bleed photography with `{typography.display-campaign}` headline set in (uppercase, 96px, line-height 0.9).
...
- Centered category illustration (~80px) + label `{typography.caption-md}` `{colors.ink}` directly below. Used in the "Latest in Clothing" 4–8-up icon strip on the men's landing page.
...
- Used in the membership page's "Member Benefits" 3-up grid.
...
- Active: identical fill with a 2px `{colors.ink}` outer ring and 2px white interior gap, creating the system's signature concentric-ring "selected" state. No size change between default and active.
...
- Layout: brand mark at left (32×32), centered nav row ("New & Featured · Men · Women · Kids · Performance · Collections · Sport"), right cluster (search pill, wishlist heart icon, bag icon).
...
**Top Nav (Mobile)**
- Hamburger menu icon (left), brand mark (center), search + bag icons (right).
...
**`faq-row`** — membership page FAQ accordion
...
### Do
- Reserve `{typography.display-campaign}` exclusively for editorial campaign hero lockups — never use 96px display type for section headers or product titles.
...
- Don't replace `{colors.ink}` with a near-black gray like `{colors.charcoal}` for a CTA — the system's primary pill is true `#111111`.
- Don't pad inside product cards. The image is full-bleed; metadata sits directly below with `{spacing.sm}` (8px) between rows.
- Don't put two campaign-tile lockups in the same row at the same scale — the system alternates a single full-bleed editorial tile with a 2-up or 4-up product/category grid.
...
| mobile | 599–320px | Single-column everything; campaign tiles render at full screen width with shorter display sizes (~64px) |

### Touch Targets
All interactive elements meet WCAG AAA (44×44px minimum). Pills (`{component.button-primary}`, `{component.button-secondary}`) sit at 48px height with 32px horizontal padding. Icon-circular buttons (`{component.button-icon-circular}`) sit at 40px — the system's PDP carousel paddle and wishlist heart sit just under AAA but above AA at 40×40, with hit-target padding extending the tappable area to 48px+. Filter-chip pills are 40px height with 16px padding.

### Collapsing Strategy
- **Primary nav:** desktop center cluster → mobile drawer triggered by hamburger at left of the brand mark.
...
3. Run `npx @google/design.md lint Subtle-Gradient-Design-System.md` after edits — `broken-ref`, `contrast-ratio`, and `orphaned-tokens` warnings flag issues automatically.
4. Add new variants as separate component entries (`-active`, `-disabled`, `-focused`) — do not bury them inside prose. The system's pressed state (`scale(0.5) opacity 0.5`) is intentional and must be its own entry, not a hover stand-in.
...
- **Mobile screenshots not captured** — responsive behavior described above synthesizes the system's known mobile pattern (hamburger drawer, 1-up grid, headline downscale) from desktop evidence and the breakpoint list extracted from tokens.
- **Hover states not documented** by system policy — the source implementation defines separate hover-state color and text tokens, but these are not included here.
